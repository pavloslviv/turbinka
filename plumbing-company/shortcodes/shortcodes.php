...
function sc_accordion($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '')
	        ...
	        . ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '')
			. '>'
...
function sc_audio($atts, $content = null) {
 ...
 . ' src="' . $src . '" class="sc_audio" ' . (sc_param_is_on($controls) ? ' controls="controls"' : '') . (sc_param_is_on($autoplay) && is_single() ? ' autoplay="autoplay"' : '') . ' width="' . $width . '" height="' . $height .'"'.($s!='' ? ' style="'.$s.'"' : '').'  '. ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'></audio>';
...
/*
[trx_button id="unique_id" type="round|round" fullsize="0|1" style="global|light|dark" size="mini|medium|big|huge|banner" icon="icon-name" link='#' target='']Button caption[/trx_button]
*/
function sc_button($atts, $content=null){	
	...
	$output = '<div class="sc_button sc_button_style_' . $style . ' ' . $type . ' sc_button_size_' . $size . ($align && $align!='none' ? ' align'.$align : '') . (!empty($class) ? ' '.$class : '')
 . ' ' . $type . 'Button' . (sc_param_is_on($fullsize) ? ' fullSize' : '') . ' ' . $style . ' ' . $size . ($icon!='' ? '  ico' : '') . '"'
		. ($s!='' ? ' style="'.$s.'"' : '') . ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '') . '>'
...
function sc_columns($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left, $width, $height);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	global $animation_all, $THEMEREX_sc_columns_count, $THEMEREX_sc_columns_counter, $THEMEREX_sc_columns_after_span2, $THEMEREX_sc_columns_after_span3, $THEMEREX_sc_columns_after_span4;
	$THEMEREX_sc_columns_counter = 1;
	$THEMEREX_sc_columns_after_span2 = $THEMEREX_sc_columns_after_span3 = $THEMEREX_sc_columns_after_span4 = false;
	$THEMEREX_sc_columns_count = $count = max(1, min(5, (int) $count));
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="columnsWrap sc_columns sc_columns_count_' . $count . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'>' . do_shortcode($content).'</div>';
...
function sc_contact_form($atts, $content = null) {
	...
	global $animation_all, $THEMEREX_ajax_nonce, $THEMEREX_ajax_url, $THEMEREX_sc_contact_form_counter, $THEMEREX_sc_contact_form_id;
	$THEMEREX_sc_contact_form_id = $id;
	$THEMEREX_sc_contact_form_counter = 0;
	$content = do_shortcode($content);
	//contact_2 = phone instead of subj
	//contact_3 = phone && email
	switch (true) {
		...
		case (!$content &&( $style == '3')):
			$form_type = 'contact_3';
			break;
		default: $form_type = 'contact_1';
	}
	$output =  '<div ' . ($id ? ' id="' . $id . '_' . $form_type . '"' : '') . ' class="sc_contact_form sc_contact_form_'.$form_type.(!empty($class) ? ' '.$class : '').'"'.($s!='' ? ' style="'.$s.'"' : '') . ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '') .'>'
		. ($title ? '<h1 class="title">' . $title . '</h1>' : '')
		. ($description ? '<div class="description">' . $description . '</div>' : '')
		. '<form' . ($id ? ' id="' . $id . '"' : '') . ' data-formtype="'.$form_type.'" method="post" action="' . ($action ? $action : $THEMEREX_ajax_url) . '">';


		if ($content != '') {
			$output .= $content;
		} else if ($style == '1') {
			$output .= '<div class="columnsWrap">' // Contact form with subject - standart
                           .'<div class="columns1_2">'
                                .'<div>'
                                    .'<input id="sc_contact_form_username" type="text" name="username" placeholder="' . __('Name', 'themerex') . '">'
                                .'</div>'
                                .'<div>'
                                    .'<input id="sc_contact_form_subj" type="text" name="phone" placeholder="' . __('Phone', 'themerex') . '">'
                                .'</div>'
                                .'<div>'
                                    .'<input id="sc_contact_form_email" type="text" name="email" placeholder="' . __('E-mail', 'themerex') . '">'
                                .'</div>'
                           .'</div>'
                           .'<div class="columns1_2">'
                               .'<div>'
                                    .'<textarea id="sc_contact_form_message" class="textAreaSize" name="message" placeholder="' . __('Message', 'themerex') . '"></textarea>'
                               .'</div>'
                            .'</div>'
                            .'<div class="columns1_2">'
                               .'<div class="sc_contact_form_button">'
                                    .'<div class="sc_button sc_button_style_dark sc_button_size_big dark big"><a href="#" class="sc_contact_form_submit">' . __('Submit', 'themerex') . '</a></div>'
                               .'</div>'
                            .'</div>'
                        .'</div>'
                        ;
		} else if ($style == '2') {
			$output .= '<div class="columnsWrap">' // Contact form with phone instead of subject
			           .'<div class="columns1_1">'
			           		.'<input id="sc_contact_form_username" type="text" name="username" placeholder="' . __('Name', 'themerex') . '">'
			           .'</div>'
                        .'<div class="columns1_1">'
                            .'<input id="sc_contact_form_phone" type="text" name="phone" placeholder="' . __('Phone', 'themerex') . '">'
                        .'</div>'
			           .'<div class="columns1_1">'
			           		.'<input id="sc_contact_form_email" type="text" name="email" placeholder="' . __('E-mail', 'themerex') . '">'
			           .'</div>'
			           .'</div>'
			           .'<div class="message">'
			           .'<textarea id="sc_contact_form_message" class="textAreaSize" name="message" placeholder="' . __('Description of work or services needed', 'themerex') . '"></textarea>'
			           .'</div>'
			           .'<div class="sc_contact_form_button">'
			           .'<div class="sc_button sc_button_style_global sc_button_size_big global big"><a href="#" class="sc_contact_form_submit">' . __('Submit', 'themerex') . '</a></div>'
			           .'</div>';
		} else if ($style == '3')if ($style == '3') {
			$output .= '<div class="columnsWrap sc_columns sc_columns_count_4">' // Contact form with phone last Name of subject
			           .'<div class="columns1_4 sc_column_item sc_column_item_1 odd first">'
				           .'<input id="sc_contact_form_username" type="text" name="username" placeholder="' . __('First Name', 'themerex') . '">'
				           .'<input id="sc_contact_form_lastname" type="text" name="subject" placeholder="' . __('Last Name', 'themerex') . '">'
			           .'</div>'
			           .'<div class="columns1_4 sc_column_item sc_column_item_2 even">'
				           .'<input id="sc_contact_form_phone" type="text" name="phone" placeholder="' . __('Your Phone', 'themerex') . '">'
				           .'<input id="sc_contact_form_email" type="text" name="email" placeholder="' . __('Your E-mail', 'themerex') . '">'
			           .'</div>'
			           
			           
			           .'<div class="columns2_4 sc_column_item sc_column_item_3 odd span_2">'			           
				           .'<div class="columnsWrap sc_columns sc_columns_count_5">'
					           .'<div class="columns3_5 sc_column_item sc_column_item_1 odd first">'
						           		.'<textarea id="sc_contact_form_message" class="textAreaSize" name="message" placeholder="' . __('Comments and Questions', 'themerex') . '"></textarea>'
					           .'</div>'
					        
				           .'<div class="columns2_5 sc_column_item sc_column_item_2 even">'   		
					           .'<div class="sc_contact_form_button">'
					           		.'<div class="sc_button sc_button_style_dark sc_button_size_huge roundButton dark huge"><a href="#" class="sc_contact_form_submit">' . __('Submit', 'themerex') . '</a></div>'
					           .'</div>'				           
				           .'</div>'
			           .'</div>'
			           .'</div>'
		...
		$output .= '<div class="result sc_infobox"></div>'
		.'</form>'
		.'</div>';
...
function sc_contact_form_item($atts, $content=null) {
	...
	$output = '<div class="sc_contact_form_field label_'.$label_position.($class ? ' '.$class : '').($align && $align!='none' ? ' align'.$align : '').'"'.($s!='' ? ' style="'.$s.'"' : '') .'>'
		...
		. ($type == 'textarea' 
			...
			: ($type=='button' 
				? '<div class="sc_contact_form_button"><div class="roundButton global ico"><a href="#" class="sc_contact_form_submit icon-comment-1">' . $value . '</a></div></div>'
...
function sc_emailer($atts, $content = null) {
	...
	$output ='<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_emailer inputSubmitAnimation' . ($align && $align!='none' ? ' sc_align' . $align : '') . (sc_param_is_on($open) ? ' sFocus rad4 opened' : ' radCircle') . (!empty($class) ? ' '.$class : '') . '"' . ($s ? ' style="'.$s.'"' : '') . ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '') . '>'
		. '<form><input type="text" class="sInput" name="email" value="" placeholder="'.__('Enter you email', 'themerex').'"></form>'
		
		.'<a href="#" class="sc_button sc_button_style_global sc_button_size_medium roundButton global medium sc_emailer_button searchIcon aIco mail" title="'.__('Submit', 'themerex').'" data-group="'.($group ? $group : __('E-mail collector group', 'themerex')).'">'.__('subscribe', 'themerex').'</a>'
		//<div class="sc_button sc_button_style_global sc_button_size_mini roundButton global mini"></div>
...
function sc_highlight($atts, $content=null){	
	...
	$s = ($color != '' ? 'color:' . $color . ';' : '')
		...
		.($style != '' ? $style : '');
	$output = '<span' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_highlight'.($type>0 ? ' sc_highlight_style_'.$type : ''). (!empty($class) ? ' '.$class : '').'"'.($s!='' ? ' style="'.$s.'"' : '').'>' . do_shortcode($content) . '</span>';
...
/*
[trx_icon id="unique_id" style='round|round' icon='' color="" bg_color="" size="" weight=""]
*/
...
function sc_image($atts, $content=null){	
	...
	if ($src > 0) {
		$attach = wp_get_attachment_image_src( $src, 'full' );
		$imageAlt = get_post_meta($src, '_wp_attachment_image_alt', true);
		if (isset($attach[0]) && $attach[0]!='')
			$src = $attach[0];
	}
	...
	$output = empty($src) ? '' : ('<figure' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_image ' . ($align && $align!='none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_'.$shape : '') . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'  '. ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'>'
				. ' <img src="' . $src . '" alt="' . $imageAlt . '" />'.(trim($title) || trim($icon) ? '<figcaption><span'.($icon ? ' class="icon '.$icon.'"' : '').'>' . $title . '</span></figcaption>' : '') 
			. '</figure>');
...
function sc_infobox($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_infobox sc_infobox_style_' . $style . (sc_param_is_on($closeable) ? ' sc_infobox_closeable' : '') . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : ''). ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'>'
			. do_shortcode($content) 
			. '</div>';
...
function sc_list($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	global $animation_all, $THEMEREX_sc_list_counter, $THEMEREX_sc_list_icon, $THEMEREX_sc_list_style;
	...
	$output = '<' . ($style=='ol' ? 'ol' : 'ul') . ($id ? ' id="' . $id . '"' : '') . ' class="sc_list sc_list_style_' . $style . (!empty($class) ? ' '.$class : '') . '"' . ($s!='' ? ' style="'.$s.'"' : '') . '  '. ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'>'
			. do_shortcode($content) 
			. '</' .($style=='ol' ? 'ol' : 'ul') . '>';
...
function sc_popup($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_popup sc_popup_light mfp-with-anim mfp-hide' . ($class ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'>'
			. do_shortcode($content) 
			. '</div>';
...
function sc_price($atts, $content=null){	
	...
	if (!empty($money)) {
		...
		if (count($m)==1) $m[1] = '';
		$output = '
			<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_price_item'. (!empty($class) ? ' '.$class : ''). ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'">
				<div class="sc_price_currency">'.$currency.'</div>
...
function sc_price_table($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	global $animation_all, $THEMEREX_sc_price_table_counter;
	$THEMEREX_sc_price_table_counter = 0;
	$count = max(1, $count);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_pricing_table columns_' . $count . ($align && $align!='none' ? ' align'.themerex_strtoproper($align) : '') . (!empty($class) ? ' '.$class : '') . '"' . ($s!='' ? ' style="'.$s.'"' : '') . '  '. ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'>'
			. do_shortcode($content)
		. '</div>';
...
function sc_price_data($atts, $content=null) {
	...
	if ($type=='price' && $money!='') {
		...
		$content = do_shortcode($content);
	$output = '<li' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_pricing_data' . ($type!='' ? ' sc_pricing_'.$type : '') . (!empty($class) ? ' '.$class : '') . '"><div>' . $content . '</div></li>';
...
function sc_quote($atts, $content=null){	
	...
	$content = do_shortcode($content);
	if (themerex_substr($content, 0, 2)!='<p') $content = '<p>' . $content . '</p>';
	$output = '<blockquote' . ($id ? ' id="' . $id . '"' : '') . $cite_param . ' class="sc_quote'. (!empty($class) ? ' '.$class : '').'"' . ($s!='' ? ' style="'.$s.'"' : '') . ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : ''). '>'
		. $content
...
function sc_skills($atts, $content=null){	
	...
	global $animation_all, $THEMEREX_sc_skills_counter, $THEMEREX_sc_skills_columns, $THEMEREX_sc_skills_height, $THEMEREX_sc_skills_max, $THEMEREX_sc_skills_dir, $THEMEREX_sc_skills_type, $THEMEREX_sc_skills_color, $THEMEREX_sc_skills_legend, $THEMEREX_sc_skills_data, $THEMEREX_sc_skills_style;
	$THEMEREX_sc_skills_counter = 0;
	$THEMEREX_sc_skills_columns = 0;
	$THEMEREX_sc_skills_height = 0;
...
function sc_skills_item($atts, $content=null) {
	...
	$title_block = '<div class="sc_skills_info">' . $title . '</div><div class="sc_skills_info">' . $stop . '%</div>';
	$old_color = $color;
...
/*
[trx_slider id="unique_id" engine="revo|flex|swiper|chop" alias="revolution_slider_alias" titles="no|slide|fixed" cat="category_id or slug" count="posts_number" ids="comma_separated_id_list" offset="" width="" height="" align="" top="" bottom=""]
...
function sc_slider($atts, $content=null){	
	...
	global $animation_all, $THEMEREX_sc_slider_engine, $THEMEREX_sc_slider_width, $THEMEREX_sc_slider_height, $THEMEREX_sc_slider_links;
...
function sc_table($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_table sc_table_style_' . max(1, min(4, $style)) . ' sc_table_size_' . $size . (!empty($align) ? ' sc_table_align_'.$align : '') . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : ''). ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '') .'>'
			. do_shortcode($content) 
			. '</div>';
...
function sc_tabs($atts, $content = null) {
	...
	$s = getStyleString($top, $right, $bottom, $left, $width);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	global $animation_all, $THEMEREX_sc_tab_counter, $THEMEREX_sc_tab_id, $THEMEREX_sc_tab_scroll, $THEMEREX_sc_tab_height, $THEMEREX_sc_tab_titles;
	$THEMEREX_sc_tab_counter = 0;
	...
	$tabs_output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_tabs sc_tabs_style_'.$style. (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '') .' data-active="' . ($initial-1). ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '') . '">'
...
function sc_tab($atts, $content = null) {
	...
	$output = '<div id="' . $id . '" class="sc_tabs_content' . ($THEMEREX_sc_tab_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_tab_counter == 1 ? ' first' : '') . (!empty($class) ? ' '.$class : '') . '">' 
...
function sc_team($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	global $animation_all, $THEMEREX_sc_team_count, $THEMEREX_sc_team_counter, $THEMEREX_sc_team_style;
    $THEMEREX_sc_team_style = $team_style ? $team_style : '';
	$THEMEREX_sc_team_count = $count = max(1, min(5, $count));
	$THEMEREX_sc_team_counter = 0;
	$content = do_shortcode($content);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_team_style_'.(esc_attr($team_style)).' sc_team'.(!empty($class) ? ' '.$class : '').'"'.($s!='' ? ' style="'.$s.'"' : '') . ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'>'
...
function sc_team_item($atts, $content=null) {
	...
	global $animation_all, $THEMEREX_sc_team_counter, $THEMEREX_sc_team_count, $THEMEREX_sc_team_style;
    //global $THEMEREX_GLOBALS;
    $THEMEREX_sc_team_style;
    $THEMEREX_sc_team_style = max(1, min(2, $THEMEREX_sc_team_style));
    //$THEMEREX_GLOBALS['sc_team_style'] = $team_style;
	...
	if (!empty($user) && $user!='none' && ($user_obj = get_user_by('login', $user)) != false) {
		...
		for ($i=0; $i<count($allowed); $i++) {
			...
			if (!empty($s[1])) {	// && array_key_exists($s[0], $THEMEREX_user_social_list)) {
				...
				$socials .= '<li><a href="' . $s[1] . '" class="social_icons icon-' . $s[0] . '" target="_blank"></a></li>';
			}
		}
	}

    if ($THEMEREX_sc_team_style == 1) {
        if (empty($photo)) {
        ...
        if (empty($photo)) {
            if (!empty($email)) $photo = get_avatar($email, 182);
        } else {
            if ($photo > 0) {
                $attach = wp_get_attachment_image_src( $photo, 'full' );
                if (isset($attach[0]) && $attach[0]!='')
            ...
            //$photo = getResizedImageTag($photo, 182, 473);
            $photo = getResizedImageTag($photo, 'auto', 473);
        }
    }
	...
	if (!empty($name)) {
        ...
        $output = '<div class="columns1_'.$THEMEREX_sc_team_count . (!empty($class) ? ' '.$class : ''). ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'">'
        ...
        . ( $THEMEREX_sc_team_style == 1 ?
            '<div class="sc_team_item_avatar">' . $photo :
            '<div class="sc_team_item_avatar">'.(strlen($link) > 0 ? '<a href="' . $link . '">'   :  ''). $photo . ((strlen($link) > 0 ? '</a>' : '') ))
         ...
         . ( $THEMEREX_sc_team_style == 1 ?
            ...
            :(
             '</div>'
            . '<div class="sc_team_item_info">'
            . '<h3 class="sc_team_item_title">' .(strlen($link) > 0 ? '<a href="' . $link . '">'   :  ''). $name . ((strlen($link) > 0 ? '</a>' : '') . '</h3>'
            . '<div class="sc_team_item_position theme_accent2">' . $position . '</div>'
            . (!empty($socials) ? '<ul class="sc_team_item_socials">' . $socials . '</ul>' : '')
            . '</div>'
            . '</div>'
...
function sc_testimonials($atts, $content=null){	
	...
	global $animation_all, $THEMEREX_sc_testimonials_count, $THEMEREX_sc_testimonials_width, $THEMEREX_sc_testimonials_height;
	$THEMEREX_sc_testimonials_count = 0;
...
function sc_testimonials_item($atts, $content=null){	
	...
	if (empty($photo)) {
		...
		$photo = getResizedImageTag($photo, 64, 64);
	}
...
function sc_text_slider($atts, $content=null){
	...
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$d = getStyleString('', '', '', '', $width, $height);
	global $animation_all, $THEMEREX_sc_text_slider_count, $THEMEREX_sc_text_slider_width, $THEMEREX_sc_text_slider_height;
	...
	return
		'<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_text_slider' . (!empty($class) ? ' '.$class : '') .'"' . ($s!='' ? ' style="'.$s.'"' : '') . ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '') . '>'
		. ($THEMEREX_sc_text_slider_count>1 ? '<div class="'.(sc_param_is_off($autoheight) ? '' : 'sc_slider_autoheight ').(sc_param_is_off($showcontrols) ? 'sc_slider_nocontrols ' : 'sc_slider_controls ').'sc_slider_pagination sc_slider sc_slider_swiper swiper-slider-container"'.($d ? ' style="'.$d.'"' : '') . ((int) $interval > 0 ? ' data-interval="'.$interval.'"' : '') . '>' : '')
...
function sc_title($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left, $width)
		...
		.($letter_spacing && $letter_spacing!='0' ? 'letter-spacing:' . $letter_spacing .'px;' : '');
...
function sc_toggles($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	global $animation_all, $THEMEREX_sc_toggle_counter, $THEMEREX_sc_toggle_style, $THEMEREX_sc_toggle_large, $THEMEREX_sc_toggle_show_counter;
	$THEMEREX_sc_toggle_counter = 0;
...
function sc_tooltip($atts, $content=null){	
	...
	$output = '<span' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_tooltip_parent'. (!empty($class) ? ' '.$class : '').'">' . do_shortcode($content) . '<span class="sc_tooltip">' . $title . '</span></span>';
...
function sc_twitter($atts, $content=null){	
	...
	if (!empty($twitter_consumer_key) && !empty($twitter_consumer_secret) && !empty($twitter_token_key) && !empty($twitter_token_secret)) {
		...
		if ($data && isset($data[0]['text'])) {
			$output = '
				<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_twitter twitBlock'.($class ? ' '.$class : '').'"'. ($s!='' ? ' style="'.$s.'"' : ''). ((sc_param_is_on($animation_all) && $animation) ? ' data-animation="animated '.$animation.'"' : '').'>
	...
	return apply_filters('themerex_shortcode_output', $output, 'trx_twitter', $atts, $content);apply_filters('themerex_shortcode_output', $output, 'trx_twitter', $atts, $content);apply_filters('themerex_shortcode_output', $output, 'trx_twitter', $atts, $content);apply_filters('themerex_shortcode_output', $output, 'trx_twitter', $atts, $content);;
}
