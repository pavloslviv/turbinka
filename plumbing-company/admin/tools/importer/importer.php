...
class TRX_Importer {
	...
	var $options = array(
        ...
        'folder_with_essgrids'  => 'essgrid',				// Name of the folder with Essential Grids data
		...
		'page_on_front'		=> 'Home 1',		            // Homepage title
...
<!--					<input type="radio" --><?php //echo ($this->options['data_type']=='no_vc'	? 'checked="checked"' : ''); ?><!-- value="no_vc" name="data_type" id="data_type_no_vc" /><label for="data_type_no_vc">--><?php //_e('Import data without Visual Composer wrappers', 'themerex'); ?><!--</label>-->
	...
	function importer() {
		...
		if ( class_exists( 'WP_Import' ) ) {
            ...
            if ($this->options['import_essgrids'])	$this->import_essgrids();
...
//        if ( ! defined( 'WPINC' ) )
//            $not_installed .= '<br>Essential Grid plugin';
        if  (!themerex_exists_universal_services_plugin())
...
if ($this->debug) echo ' '.__('import error', 'themerex').'<br>';
    ...
    function import_essgrids() {
        if (file_exists(WP_PLUGIN_DIR.'/essential-grid/essential-grid.php')) {
            ...
            $dir = get_template_directory().'/admin/tools/importer/data/essgrid';
            if ( is_dir($dir) ) {
                $hdir = @opendir( $dir );
                if ( $hdir ) {
