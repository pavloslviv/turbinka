...
if ( !function_exists( 'themerex_theme_setup' ) ) {
	function themerex_theme_setup() {
		...
		// Non 16:9
		add_image_size( 'classic3',  400, 248, true );
...
if ( !function_exists( 'themerex_scripts' ) ) {
	function themerex_scripts() {
		...
		$concatenate_scripts = $THEMEREX_DEBUG_MODE=='no' && get_theme_option('compose_scripts')=='yes';
		...
		if ($THEMEREX_DEBUG_MODE=='no' && get_theme_option('packed_scripts')=='yes' && file_exists(themerex_get_file_dir('/css/__packed.css'))) {
			// Load packed styles
			...
			// Animations
			if (get_theme_option('css_animation'))
		...
		else if ($THEMEREX_DEBUG_MODE=='no' && get_theme_option('packed_scripts')=='yes' && file_exists(themerex_get_file_dir('/css/__packed.css')))
		...
		if ($THEMEREX_DEBUG_MODE=='no' && get_theme_option('packed_scripts')=='yes' && file_exists(themerex_get_file_dir('/js/__packed.js'))) {
...
if ( !function_exists( 'themerex_compose_scripts' ) ) {
	function themerex_compose_scripts() {
		...
		$THEMEREX_DEBUG_MODE = 'yes';
		if (is_admin() || $THEMEREX_DEBUG_MODE=='yes' || get_theme_option('compose_scripts')!='yes' || !is_object($wp_scripts)) return;
...
if ( !function_exists( 'themerex_footer_scripts' ) ) {
	function themerex_footer_scripts() {
		$THEMEREX_DEBUG_MODE = 'yes';
		if (is_admin() || $THEMEREX_DEBUG_MODE=='yes' || get_theme_option('compose_scripts')!='yes') return;
...
if ( !function_exists( 'themerex_compose_styles' ) ) {
	function themerex_compose_styles() {
		...
		$THEMEREX_DEBUG_MODE = 'yes';
		if (is_admin() || $THEMEREX_DEBUG_MODE=='yes' || get_theme_option('compose_scripts')!='yes' || !is_object($wp_styles)) return;
...
function themerex_init_template() {
	...
	global $logo_text, $logo_slogan, $logo_icon, $logo_image, $logo_side, $logo_fixed, $logo_footer, $acc_icon;
	...
	if ($acc_icon=='' && ($logo_image = get_custom_option('accredited_icon')) == '' && file_exists(themerex_get_file_dir('/skins/' . $skin . '/images/acc-icon.png')))
		$acc_icon = themerex_get_file_url('/skins/' . $skin . '/images/acc-icon.png');
...
if ( !function_exists( 'themerex_callback_send_contact_form' ) ) {
	function themerex_callback_send_contact_form() {
		...
		if (!($contact_email = get_theme_option('contact_email')) && !($contact_email = get_theme_option('admin_email'))) 
			...
			} elseif ($type=='contact_2') {
				$user_name = themerex_substr($post_data['username'], 0, 20);
				$user_email = themerex_substr($post_data['email'], 0, 60);
				$user_phone = getShortString($post_data['phone'], 100);
				$user_msg = getShortString($post_data['message'], 300);
...
require_once( themerex_get_file_dir('plugin.universal_services_plugin.php') );
