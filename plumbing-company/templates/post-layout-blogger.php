...
if ($opt['style'] == 'list') {
	...
	$info = '';
	$info = sc_param_is_on($opt['info']) && sc_param_is_on($opt['loadmore']) ? '<div class="sc_blogger_info">'
		. (themerex_strpos($opt['style'], 'image')!==false || themerex_strpos($opt['style'], 'accordion')!==false
			? '<div class="sc_blogger_more sc_button sc_button_style_global sc_button_size_medium roundButton global medium"><a title="" href="'.esc_url($post_data['post_link']).'">'.($opt['readmore'] ? esc_html($opt['readmore']) : __('More', 'themerex')).'</a></div><div class="sc_blogger_author">' . __('Posted by', 'themerex')
			: __('by', 'themerex'))
	...
	$info .= sc_param_is_on($opt['info']) && sc_param_is_off($opt['loadmore']) ? '<div class="sc_blogger_info">'
		. (themerex_strpos($opt['style'], 'image')!==false || themerex_strpos($opt['style'], 'accordion')!==false
	...
	$info .= sc_param_is_off($opt['info']) && sc_param_is_on($opt['loadmore']) ? '<div class="sc_blogger_info">'
		. (themerex_strpos($opt['style'], 'image')!==false || themerex_strpos($opt['style'], 'accordion')!==false
			? '<div class="sc_blogger_more sc_button sc_button_style_global sc_button_size_medium roundButton global medium"><a title="" href="'.esc_url($post_data['post_link']).'">'.($opt['readmore'] ? esc_html($opt['readmore']) : __('More', 'themerex')).'</a></div>'
			: '')
		. '</div>'
		: '';
...
<article class="sc_blogger_item<?php
	...
	?>
		<div class="sc_blogger_date">
			<span class="day_month"><?php echo esc_html($post_data['post_date_part1']); ?></span>
			<span class="year"><?php echo esc_html($post_data['post_date_part2']); ?></span>
		</div>
	<?php 
	}
