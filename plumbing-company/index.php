    ...
    if (get_custom_option('show_filters')=='yes') {
        if (get_custom_option('filter_taxonomy')=='categories') {			// Use categories as filter items
            ...
            if (count($portfolio_list) > 0) {
                $filters .= '<li class="roundButton'.($portfolio_parent==$cat_id ? ' active' : '').'"><a href="#" data-filter="*">'.__('All', 'themerex').'</a></li>';
                foreach ($portfolio_list as $cat) {
                    $filters .= '<li class="roundButton'.($cat->term_id==$cat_id ? ' active' : '').'"><a href="#" data-filter=".flt_'.$cat->term_id.'">'.$cat->name.'</a></li>';
                }
            }
        } else {															// Use tags as filter items
            if (count($flt_ids) > 0) {
                $filters .= '<li class="roundButton active"><a href="#" data-filter="*">'.__('All', 'themerex').'</a></li>';
                foreach ($flt_ids as $flt_id=>$flt_name) {
                    $filters .= '<li class="roundButton"><a href="#" data-filter=".flt_'.$flt_id.'">'.$flt_name.'</a></li>';
                }
            }
        }
        if ($filters) {
...
if ($post_number > 0) {
    ...
    if (in_array($pagination, array('viewmore', 'infinite'))) {
        if ($page_number < $wp_query->max_num_pages) {
            ?>
			<div id="viewmore" class="roundButton pagination_<?php echo esc_attr($pagination); ?>">
        ...
        showPagination(array(
                ...
                'button_class' => 'roundButton light',
