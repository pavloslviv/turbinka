		...
		foreach ($taxes as $tax) {
			...
			if (!$list) {
				...
				if (count($terms) > 0) {
					...
					$list .= "<select name='".esc_attr($tax_slug)."' id='".esc_attr($tax_slug)."' class='postform'>"
...
if (!function_exists('showBreadcrumbs')) {
	function showBreadcrumbs($args=array()) {
		...
		$args['delimiter'] = '<span class="breadcrumbs_delimiter">'. esc_html($args['delimiter']) .'</span>';
		if ( !in_array($type, array('home', 'frontpage')) ) {
			...
			if ($type == 'page' || $type == 'attachment') {
				...
				while ($pageParentID > 0) {
					...
					if ($level > $args['max_levels'])
						...
						$rez2 = '<a class="cat_post" href="' . esc_url(get_permalink($pageParent->ID)) . '">' . esc_html(getShortString($pageParent->post_title, $args['truncate_title'], $args['truncate_add'])) . '</a>' . (!empty($rez2) ? balanceTags($args['delimiter']) : '') . $rez2;
						...
						if (!in_array($type, array('woocommerce_product', 'tribe_event')) && $args['show_all_filters']) {
							...
							if (($tpl_id = getTemplatePageId('only-'.$post_format)) > 0) {
								...
								if ($level > $args['max_levels'])
									...
									$rez_all .= (!empty($rez_all) ? balanceTags($args['delimiter']) : '') . '<a class="all" href="' . esc_url(get_permalink($tpl_id)) . '">' . esc_html(sprintf(__('All %s', 'themerex'), getPostFormatName($post_format, false))) . '</a>';
							}
						}
						...
						if ($cat) {
							if (!in_array($cat['term_id'], $ex_cats)) {
								...
								if ($level > $args['max_levels'])
									...
									$rez2 = '<a class="cat_post" href="' . esc_url($cat_link) . '">' . esc_html(getShortString($cat['name'], $args['truncate_title'], $args['truncate_add'])) . '</a>' . (!empty($rez2) ? balanceTags($args['delimiter']) : '') . $rez2;
							}
						} else {
							$post_type = get_post_type($post_id);
							$parentTax = 'category' . ($post_type == 'post' ? '' : '_' . esc_attr($post_type));
						}
					...
					if ($cat) {
						if (!in_array($cat['term_id'], $ex_cats)) {
							...
							if ($level > $args['max_levels'])
								...
								$rez2 = '<a class="cat_parent" href="' . esc_url($cat_link) . '">' . esc_html(getShortString($cat['name'], $args['truncate_title'], $args['truncate_add'])) . '</a>' . (!empty($rez2) ? balanceTags($args['delimiter']) : '') . $rez2;
						}
						$parent = $cat['parent'];
					}
				}
			...
			if (themerex_strpos($type, 'woocommerce')!==false && ($shop_id=get_option('woocommerce_shop_page_id'))>0 && !in_array(themerex_strtolower($title), array(themerex_strtolower($shop_title=getPostTitle($shop_id))))) {
				$rez_all = '<a class="all" href="' . esc_url(get_permalink($shop_id)) . '">' . esc_html($shop_title) . '</a>' . (!empty($rez_all) ? balanceTags($args['delimiter']) : '') .  $rez_all;
			}
			if (themerex_strpos($type, 'tribe')!==false && !in_array(themerex_strtolower($title), array(__( 'All Events', 'themerex' ), __( 'Tribe Events', 'themerex' )))) {
				$rez_all = '<a class="all" href="' . esc_url(tribe_get_events_link()) . '">' . __( 'All Events', 'themerex') . '</a>' . (!empty($rez_all) ? balanceTags($args['delimiter']) : '') . $rez_all;
			}
			if ($args['show_all_posts'] && !in_array(themerex_strtolower($title), array(themerex_strtolower(__( 'All Posts', 'themerex' )))) && ($blog_id = getTemplatePageId('blog')) > 0) {
				$rez_all = '<a class="all" href="' . esc_url(get_permalink($blog_id)) . '">' . __( 'All Posts', 'themerex') . '</a>' . (!empty($rez_all) ? balanceTags($args['delimiter']) : '') . $rez_all;
			}
	
			$rez3 = '';
			if ($type == 'tribe_day' && is_object($post)) {
				$rez3 .= (!empty($rez3) ? balanceTags($args['delimiter']) : '') . '<a class="cat_parent" href="' . esc_url(tribe_get_gridview_link(false)) . '">' . esc_html(date_i18n(tribe_get_option('monthAndYearFormat', 'F Y' ), strtotime(tribe_get_month_view_date()))) . '</a>';
			} else if (themerex_strpos($type, 'woocommerce')===false && is_archive() && is_object($post)) {
				$year  = get_the_time('Y'); 
				$month = get_the_time('m'); 
				if (is_day() || is_month())
					$rez3 .= (!empty($rez3) ? balanceTags($args['delimiter']) : '') . '<a class="cat_parent" href="' . esc_url(get_year_link( $year )) . '">' . esc_html($year) . '</a>';
				if (is_day())
					$rez3 .= (!empty($rez3) ? balanceTags($args['delimiter']) : '') . '<a class="cat_parent" href="' . esc_url(get_month_link( $year, $month )) . '">' . esc_html(prepareDateForTranslation(get_the_date( 'F' ))) . '</a>';
			}
	
	
			if (!is_front_page()) {	// && !is_home()
				$rez .= (isset($args['home']) && $args['home']!='' ? '<a class="home" href="' . ($args['home_url'] ? esc_url($args['home_url']) : esc_url(home_url())) . '">' . esc_html($args['home']) . '</a>' . balanceTags($args['delimiter']) : '')
					. (!empty($rez_all) ? $rez_all . balanceTags($args['delimiter']) : '')
					. (!empty($rez_level) ? $rez_level . balanceTags($args['delimiter']) : '')
					. (!empty($rez2)    ? $rez2 . balanceTags($args['delimiter']) : '')
					. (!empty($rez3)    ? $rez3 . balanceTags($args['delimiter']) : '')
					. ($title ? '<span class="current">' . esc_html($title) . '</span>' : '');
			}
		}
	...
	function showSinglePageNav( $args ) {
		...
		<nav role="navigation" id="<?php echo esc_attr( $args['id'] ); ?>" class="<?php echo esc_attr($args['class']); ?>">
			...
			<?php if ( is_single() ) : // navigation links for single posts ?>
				<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . esc_html($args['prev_text']) . '</span> %title' ); ?>
				<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . esc_html($args['next_text']) . '</span>' ); ?>
			...
			if ($max_page > 1) {
				...
				if ($opt['style'] == 'pages') {
					...
					if ($opt['pages_text']) {
						...
						$output .= '<li class="pager_pages '.esc_attr($opt['button_class']).'"><span>' . esc_html($pages_text) . '</span></li>';
					}
					if ($cur_page > 1) {
						// First page
						$page_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), $opt['first_text']);
						$output .= '<li class="pager_first '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link()).'">'.esc_html($page_text).'</a></li>';
						// Prev page
						$output .= '<li class="pager_prev '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($cur_page-1)).'">'.esc_html($opt['prev_text']).'</a></li>';
					}
					// Page buttons
					$group = 1;
					$dot1 = $dot2 = false;
					for ($i = 1; $i <= $max_page; $i++) {
						if ($i % $opt['pages_in_group'] == 1) {
							...
							if ($group != $cur_group)
								$output .= '<li class="pager_group '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($i)).'">'.intval($i).'-'.min($i+$opt['pages_in_group']-1, $max_page).'</a></li>';
						}
						if ($group == $cur_group) {
							if ($i < $show_pages_start) {
								if (!$dot1) {
									$output .= '<li class="pager_dot '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($show_pages_start-1)).'">'.esc_html($opt['dot_text']).'</a></li>';
									$dot1 = true;
								}
							} else if ($i > $show_pages_end) {
								if (!$dot2) {
									$output .= '<li class="pager_dot '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($show_pages_end+1)).'">'.esc_html($opt['dot_text']).'</a></li>';
									$dot2 = true;
								}
							} else if ($i == $cur_page) {
								$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $opt['current_text']);
								$output .= '<li class="pager_current active '.esc_attr($opt['button_class']).'"><span>'.esc_html($page_text).'</span></li>';
							} else {
								$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $opt['page_text']);
								$output .= '<li class="'.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($i)).'">'.esc_html($page_text).'</a></li>';
							}
						}
					}
					if ($cur_page < $max_page) {
						// Next page
						$output .= '<li class="pager_next '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($cur_page+1)).'">'.esc_html($opt['next_text']).'</a></li>';
						// Last page
						$page_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), $opt['last_text']);
						$output .= '<li class="pager_last '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($max_page)).'">'.esc_html($page_text).'</a></li>';
					}
					...
					if ($cur_page > 1) {
						...
						$output .= '<li class="pager_first ico left '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link()).'">'.esc_html($page_text).'</a></li>';
						// Prev page
						$page_text = str_replace("&laquo;", '', $opt['prev_text']);
						$output .= '<li class="pager_prev ico left '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($cur_page-1)).'">'.esc_html($page_text).'</a></li>';
					}
					// Page XX from XXX
					...
					$pages_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), $pages_text);
					$output .= '<li class="pager_pages libPage">' . balanceTags($pages_text);
					// Page buttons
					$output .= '<div id="pageNavSlider" class="boxShadow pageFocusBlock navPadding">'
						...
						. '<ul class="slides swiper-wrapper" data-current-slide="'.esc_attr($cur_group).'">';
					$group = 1;
					$row_opened = false;
					for ($i = 1; $i <= $max_page; $i++) {
						if ($i % $opt['pages_in_group'] == 1) {
					...
					if ($cur_page < $max_page) {
						...
						$output .= '<li class="pager_next ico right '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($cur_page+1)).'">'.esc_html($page_text).'</a></li>';
						// Last page
						$page_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), $opt['last_text']);
						$page_text = str_replace("&raquo;", '', $page_text);
						$output .= '<li class="pager_last ico right '.esc_attr($opt['button_class']).'"><a href="'.esc_url(get_pagenum_link($max_page)).'">'.esc_html($page_text).'</a></li>';
					}
...
if (!function_exists('getThumbSizes')) {
	function getThumbSizes($opt) {
		...
		$thumb_sizes = array(
			...
			'related'           => array('w' => 400, 'h' => $opt['thumb_crop'] ? 248 : null, 'h_crop' => 248),	// 400 x 248
			...
			'image_large'       => array('w' => 350, 'h' => $opt['thumb_crop'] ? 252 : null, 'h_crop' => 248),	// 350 x 252
			'masonry2'          => array('w' => 714, 'h' => $opt['thumb_crop'] ? 402 : null, 'h_crop' => 402),	// 550 x 309
			'masonry3'          => array('w' => 350, 'h' => $opt['thumb_crop'] ? 252 : null, 'h_crop' => 248),	// 350 x 252
			...
			'classic3'          => array('w' => 350, 'h' => $opt['thumb_crop'] ? 252 : null, 'h_crop' => 248),	// 350 x 252
			...
			// Non 16:9
			'portfolio3'   => array('w' => 350, 'h' => $opt['thumb_crop'] ? 252 : null, 'h_crop' => 245),	// 350 x 252
	...
	function getPostData(&$opt, $post_obj=null) {
		...
		if ($post_format == 'gallery') {
			...
			if ($post_video=='') {
				...
				if ($src) $post_video = substituteVideo('<video src="'.esc_url($src).'">', $thumb_sizes['w'], round($thumb_sizes['w']/16*9), false);	//$thumb_sizes['h_crop']);
			}
			...
			if ($post_audio=='') {
				...
				if ($src) $post_audio = substituteAudio('<audio src="'.esc_url($src).'"></audio>');
			}
		...
		if ($opt['categories_list']) {
			...
			for ($i = 0; $i < count($post_categories_list); $i++) {
				...
				$post_categories_links .= '<a class="cat_link" href="' . esc_url($post_categories_list[$i]['link']) . '">'
					. esc_html($post_categories_list[$i]['name'])
		...
		if ($opt['tags_list']) {
			if (($post_tags_list = get_the_tags()) != 0) {
				...
				foreach ($post_tags_list as $tag) {
					$tag_number++;
					$post_tags_links .= '<a class="tag_link" href="' . esc_url(get_tag_link($tag->term_id)) . '">' . esc_html($tag->name) . ($tag_number==count($post_tags_list) ? '' : ',') . '</a> ';
			...
			foreach ($photos as $photo) {
				...
				if ($gallery_items_in_bg) {
					...
					$gallery_text .= '<li'
						...
						. (!empty($h) ? 'height:' . esc_attr($h) . (themerex_strpos($h, '%')!==false ? '' : 'px').';' : '')
						. '">'
						. ($zoom ? '<a href="'. esc_url($photo) . '">'.esc_html($photo_min).'</a>'
								 : (!empty($link) ? '<a href="'. esc_url($link) . '">'.balanceTags($photo_min).'</a>' : balanceTags($photo_min)))
						. '</li>';
				}
			}
			$gallery_text .= '</ul>';
	...
	function substituteAudio($post_text) {
		...
		while (($pos_start = themerex_strpos($post_text, themerex_substr($tag, 0, -1).' ', $pos_start+1))!==false) {
			...
			if ($src != '') {
				...
				$container = '<div'.($id ? ' id="'.esc_attr($id).'"' : '').' class="audio_container' . ($tag_a ? ' align'.esc_attr($tag_a) : '') . '"' . ($tag_s || $tag_w || $tag_h ? ' style="'.($tag_w!='' ? 'width:' . esc_attr($tag_w) . (themerex_substr($tag_w, -1)!='%' ? 'px' : '') . ';' : '').($tag_h!='' ? 'height:' . esc_attr($tag_h) . 'px;' : '') . esc_attr($tag_s) . '"' : '') . '>';
				$post_text = themerex_substr($post_text, 0, (themerex_substr($post_text, $pos_start-3, 3)=='<p>' ? $pos_start-3 : $pos_start)) 
					...
					. (themerex_strpos($src, 'soundcloud.com') !== false 
						? '<iframe width="100%" height="166" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url='.esc_attr($src).'"></iframe>'
						: $tag_text)
					. '</div>'
	...
	function substituteVideo($post_text, $w, $h, $in_frame=true) {
		...
		while (($pos_start = themerex_strpos($post_text, themerex_substr($tag, 0, -1).' ', $pos_start+1))!==false) {
			...
			if ($src != '') {
				...
				$tag_a = getTagAttrib($tag_text, $tag, 'align');
				$tag_s = getTagAttrib($tag_text, $tag, 'style');
				$video = '<iframe'.($id ? ' id="'.esc_attr($id).'"' : '').' class="video_frame' . ($tag_a ? ' align'.esc_attr($tag_a) : '') . '"'
					. ' src="' . esc_url($src) . '"'
					. ' width="' . ($tag_w ? esc_attr($tag_w) : esc_attr($w)) . '"'
					. ' height="' . ($tag_h ? esc_attr($tag_h) : esc_attr($h)) . '"'
...
if (!function_exists('getVideoFrame')) {
	function getVideoFrame($video, $image='', $title=false, $style='') {
		...
		$tag_anim = getTagAttrib($video, $tag, 'data-animation');
		$html = '<div class="sc_video_player"' . ' data-width="'.esc_attr($tag_w).'" data-height="'.esc_attr($tag_h).'"'.($style ? ' style="' . esc_attr($style) . '"' : '')
...
if (!function_exists('showShareSocialLinks')) {
	function showShareSocialLinks($args) {
		$args = array_merge(array(
			...
			'use_icons' => true,				// use font icons or images
		...
		foreach ($args['share'] as $s => $data) {
			...
			if (!empty($data['icon'])) {
				...
				else $sn = str_replace('icon-','',$data['icon']);
			}
			else return;
			$link = str_replace(
			...
			$output .= (in_array($args['style'], array('list', 'drop')) ? '<li>' : '') 
				. '<a href="' . ($args['popup'] ? '#' : esc_attr($link)) . '" class="share-item' . ($args['use_icons'] ? ' social_icons ' . $data['icon'] : '').'"'
				...
				. '>'
				. ($args['use_icons'] ? strpos($sn, 'http://') !== false ? '<img src="'.$sn.'" alt="">' : $sn : '<img src="'.$data['icon'].'" alt="' . $s . '">')
				. ($args['use_icons'] ? '' : ($args['style']=='drop' ? themerex_strtoproper($s) : '') )
				. '</a>'
		...
		foreach ($args['allowed'] as $s) {
			if (array_key_exists($s, $THEMEREX_user_social_list)) {
				...
				if ($link) {
					...
					$output .= $args['before']
						. '<a href="' . esc_url($link) . '" class="social_icons icon-' . esc_attr($s) . '" target="_blank"></a>'
						. $args['after'];
				}
			}
		}
	...
	?>
		...
		?>
			<tr>
				...
				<td><input type="text" name="user_<?php echo esc_attr($name); ?>" id="user_<?php echo esc_attr($name); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.esc_attr($name), $user->ID)); ?>" />
